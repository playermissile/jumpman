...
@cont:  jsr copy_level_to_memory

        lda $2800
        bpl @v1
        jsr change_v2_locations
...
@save_level:            ; first time through, save the level for replay
        ...
        ldx #$08
        jsr copypg
        lda #$a8        ; copy Block B to storage for replay
        ldy #$98
        ldx #$10
        jsr copypg
...
@load_level:            ; after first time, load pristine copy of level from storage
        ...
        ldx #$08
        jsr copypg
        lda #$98        ; copy Block B storage to $a800
        ldy #$a8
        ldx #$10
        jsr copypg
        rts

v2_level_number:        ; 2801 is already loaded here
        jsr hex2level
        sta $4636
        stx $4637
        rts
...
change_v2_locations:
        lda #$28        ; change title lookup and level sanity check
        sta $4545
        sta $454c
        sta $4554
        lda #$6b
